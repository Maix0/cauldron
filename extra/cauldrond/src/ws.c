...
/* Link client_socks records via next_in_group by group_key.
 */
static void update_group_links(char *group_key) {
	int i, prev_client = -1, first_client = -1;
	...
	for (i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (client_socks[i].group_key == NULL)
			continue;

		if (strcmp(client_socks[i].group_key, group_key) != 0)
			continue;
		...
		if (prev_client != -1)
			client_socks[prev_client].next_in_group = i;

		prev_client = i;
	}

	if (prev_client != -1)
		client_socks[prev_client].next_in_group = first_client;
 ...
 * @param echo      Send frame back to sender
 ...
 */
int ws_sendframe(int fd, const char *msg, uint64_t size, bool broadcast, bool echo, int type)
{
	...
	output = echo ? SEND(fd, response, idx_response) : 0;
...
/* Send frame to group members
 */
int ws_sendframe_group(uint64_t sender_index, const char *msg, int size, int type)
{
	int fd;
	unsigned char *response; /* Response data.     */
	unsigned char frame[10]; /* Frame.             */
	...
	fd = client_socks[sender_index].client_sock;
	(void)(fd);
	frame[0] = (WS_FIN | type);
	length   = (uint64_t)size;
	...
	if (client_socks[sender_index].group_key == NULL)
		goto no_sending;

	i = client_socks[sender_index].next_in_group;
	while (i != sender_index) {
		if ((send_ret = SEND(client_socks[i].client_sock, response, idx_response)) != -1)
			output += send_ret;
		else
		{
			output = -1;
		...
		i = client_socks[i].next_in_group;
	}
...
static int do_handshake(struct ws_frame_data *wfd, int p_index)
{
	...
	if (ports[p_index].events.onopen != NULL)
	{
...
static int do_close(struct ws_frame_data *wfd, int close_code)
{
	...
	custom_close:
		...
		if (ws_sendframe(CLI_SOCK(wfd->sock), (const char *)wfd->msg_ctrl,
				sizeof(char) * 2, false, true, WS_FR_OP_CLSE) < 0)
		{
...
send:
	if (ws_sendframe(CLI_SOCK(wfd->sock), (const char *)wfd->msg_ctrl,
			wfd->frame_size, false, true, WS_FR_OP_CLSE) < 0)
	{
		DEBUG("An error has occurred while sending closing frame!\n");
		return (-1);
	}
...
static void *ws_establishconnection(void *vsock)
{
	...
	while (next_frame(&wfd, connection_index) >= 0)
	{
		...
		if ((wfd.frame_type == WS_FR_OP_TXT || wfd.frame_type == WS_FR_OP_BIN) &&
			!wfd.error)
		{
			if (strncmp((char*)wfd.msg, "{\"group_key\":\"", 14) == 0)
			{
				if ((pos = strchr((char*)wfd.msg + 14, '"')) != NULL) {
					*pos = '\0';
					client_socks[connection_index].group_key = strdup((char*)wfd.msg + 15);
					update_group_links(client_socks[connection_index].group_key);
				}
			}
			else
			{
				ws_sendframe_group(connection_index, (char*)wfd.msg, wfd.frame_size, wfd.frame_type);
				if (ports[p_index].events.onmessage != NULL)
				{
					ports[p_index].events.onmessage(sock, wfd.msg, wfd.frame_size, wfd.frame_type);
				}
			}
		}
	 ...
	 */
	if (ports[p_index].events.onclose != NULL)
	{
...
closed:
	...
	if (client_socks[connection_index].state != WS_STATE_CLOSED)
	{
		...
		if (client_socks[connection_index].group_key != NULL)
		{
			group_key = client_socks[connection_index].group_key;
			client_socks[connection_index].group_key = NULL;
			update_group_links(group_key);
			free(group_key);
		}
		close_socket(sock);
	...
	while (1)
	{
		...
		for (i = 0; i < MAX_CLIENTS; i++)
		{
			if (client_socks[i].client_sock == -1)
			{
				...
				client_socks[i].group_key     = NULL;
				client_socks[i].next_in_group = -1;
